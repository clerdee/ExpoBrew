...
import {API_BASE_URL} from './src/configs/config';

Notifications.setNotificationHandler({handleNotification:async()=>({shouldShowAlert:true,shouldPlaySound:true,shouldSetBadge:false})});
const Stack=createStackNavigator();

export default function App(){
  const [isLoading,setIsLoading]=useState(true);const [initialRoute,setInitialRoute]=useState("Auth");
  ...
  useEffect(()=>{
    const init=async()=>{
      try{
        ...
        if(userToken&&userInfoString){
          ...
          // Only attempt to get push tokens on physical devices to prevent emulator crashes
          if (Device.isDevice) {
            registerForPushNotificationsAsync().then(token=>{
              if(token) axios.post(`${API_BASE_URL}/users/push-token`,{token},{headers:{Authorization:`Bearer ${userToken}`}}).catch(()=>{});
            });
          }
    ...
    const sub=Notifications.addNotificationResponseReceivedListener(response=>{
      ...
      if(data?.orderId&&navigationRef.current) navigationRef.current.navigate('Home',{screen:'OrderDetailPage',params:{orderId:data.orderId}});
    });
    return ()=>sub.remove();
  },[]);
  ...
  return(
    <ReduxProvider store={store}><PaperProvider><NavigationContainer ref={navigationRef}>
    <Stack.Navigator initialRouteName={initialRoute} screenOptions={{headerShown:false}}>
      <Stack.Screen name="Auth" component={AuthStackNavigator}/><Stack.Screen name="Home" component={UserStackNavigator}/><Stack.Screen name="AdminHome" component={AdminStackNavigator}/>
    </Stack.Navigator></NavigationContainer><Toast/></PaperProvider></ReduxProvider>
  );
}
...
async function registerForPushNotificationsAsync(){
  ...
  if(Device.isDevice){
    ...
    try {
      ...
      console.log("Push token error (Safe to ignore on emulator):", error.message);
    }
  }
  return token;
}
