...
const updateUserProfile=async(req,res)=>{try{const user=await User.findById(req.user._id);if(!user)return res.status(404).json({message:'Not found'});const {name,email,phone,birthday,addresses,currentPassword,newPassword}=req.body;if(newPassword){if(!currentPassword)return res.status(400).json({message:'Current password required'});const isMatch=await bcrypt.compare(currentPassword,user.password);if(!isMatch)return res.status(400).json({message:'Invalid password'});user.password=await bcrypt.hash(newPassword,10);}if(req.file){if(user.profileImageId)await cloudinary.uploader.destroy(user.profileImageId);const result=await cloudinary.uploader.upload(req.file.path,{folder:'ExpoBrew/Profiles'});user.profileImage=result.secure_url;user.profileImageId=result.public_id;}if(name)user.name=name;if(email)user.email=email;if(phone!==undefined)user.phone=phone;if(birthday!==undefined)user.birthday=birthday;if(addresses)user.addresses=typeof addresses==='string'?JSON.parse(addresses):addresses;await user.save();res.status(200).json(buildUserPayload(user));}catch(e){res.status(500).json({message:'Error'});}};
const toggleFavorite=async(req,res)=>{try{const user=await User.findById(req.user._id);const productId=req.params.productId;if(user.favorites.includes(productId)){user.favorites=user.favorites.filter(id=>id.toString()!==productId);await user.save();return res.status(200).json({message:'Removed',favorites:user.favorites});}user.favorites.push(productId);await user.save();return res.status(200).json({message:'Added',favorites:user.favorites});}catch(e){res.status(500).json({message:'Error'});}};
