...
const createReview = async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(403).json({ message: 'Only completed purchased products can be reviewed.' });
    }

    const purchasedItem = order.orderItems.find(item =>
      (item.product && item.product.toString() === productId) || item.name === product.name
    );

    if (!purchasedItem) {
      return res.status(403).json({ message: 'You can only review products from your completed orders.' });
    }

    const existingReview = await Review.findOne({ user: req.user._id, product: productId, order: orderId });
    if (existingReview) {
      return res.status(400).json({ message: 'You already reviewed this product for that order.', reviewId: existingReview._id });
    }
    ...
    if (e.code === 11000) {
      return res.status(400).json({ message: 'You already reviewed this product for that order.' });
    }
