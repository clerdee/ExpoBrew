...
const verifyOtp = async (req, res) => {
  try {
    ...
    const user = await User.create({
      name: temp.name, email: temp.email, password: await bcrypt.hash(temp.password, await bcrypt.genSalt(10)), 
      profileImage: req.file ? req.file.path : null, profileImageId: req.file ? req.file.filename : null 
    });
...
const loginUser = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Login successful!", token, user: { id: user._id, name: user.name, email: user.email, role: user.role, profileImage: user.profileImage } });
