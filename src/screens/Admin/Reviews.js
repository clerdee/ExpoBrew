...
import { Text, Card, IconButton, ActivityIndicator, Searchbar, Avatar, Divider, Button } from 'react-native-paper';
...
export default function Reviews({ navigation }) {
  ...
  const filtered = useMemo(() => reviews.filter(i => {
    const name = (i.user?.name || '').toLowerCase(), email = (i.user?.email || '').toLowerCase();
    const prod = (i.product?.name || '').toLowerCase(), comm = (i.comment || '').toLowerCase(), term = search.toLowerCase();
        ...
        <View style={styles.cardInner}>
          <View style={styles.cardContent}>
            <View style={styles.sideHeader}>
              {i.user?.profileImage ? <Image source={{ uri: i.user.profileImage }} style={styles.userImage} /> : <Avatar.Text size={45} label={i.user?.name?.charAt(0) || 'U'} style={styles.avatar} color="#FFF" />}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FAF5F0' }}>
      <View style={styles.header}>
        ...
        <Searchbar placeholder="Search name, email, or coffee..." onChangeText={setSearch} value={search} style={styles.searchBar} inputStyle={{ fontSize: 14 }} iconColor="#4A2E1B" elevation={2} />
      </View>
      ...
      {loading && !refreshing ? <ActivityIndicator size="large" color="#4A2E1B" style={{ flex: 1 }} /> : (
        <FlatList 
          data={filtered} keyExtractor={i => i._id} renderItem={renderItem} contentContainerStyle={styles.list} 
...
const styles = StyleSheet.create({
  ...
  filters: { paddingHorizontal: 20, marginTop: 40, marginBottom: 10 }, filterTitle: { fontSize: 11, fontWeight: '800', color: '#8B5E3C', textTransform: 'uppercase', marginBottom: 10, letterSpacing: 0.5 },
  ...
  pillOn: { backgroundColor: '#6F4E37', borderColor: '#6F4E37' }, filterText: { fontSize: 12, color: '#666', fontWeight: 'bold' }, pillTxtOn: { color: '#FFF' },
  list: { padding: 16, paddingBottom: 40 }, 
  card: { marginBottom: 16, backgroundColor: '#FFF', borderRadius: 20, elevation: 3 },
  cardInner: { borderRadius: 20, overflow: 'hidden' },
  cardContent: { flexDirection: 'row', padding: 15 }, sideHeader: { alignItems: 'center' }, 
  userImage: { width: 45, height: 45, borderRadius: 22.5, backgroundColor: '#EEE' }, avatar: { backgroundColor: '#D2B48C' },
  ...
  info: { flex: 1, marginLeft: 15 }, titleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  name: { fontSize: 15, fontWeight: 'bold', color: '#333' }, emailText: { fontSize: 11, color: '#888', marginTop: 1 },
  dateTime: { fontSize: 11, color: '#AAA', marginTop: 5, fontWeight: '600' }, cardDiv: { marginVertical: 10, opacity: 0.3 }, 
  productRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 }, productName: { fontSize: 12, fontWeight: '900', color: '#6F4E37', marginLeft: 6 },
