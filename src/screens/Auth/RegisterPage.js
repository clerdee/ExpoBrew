...
import { Alert, KeyboardAvoidingView, Linking, Modal, Platform, ScrollView, StyleSheet, TouchableOpacity, View } from 'react-native';
...
const RegisterPage = ({ navigation }) => {
  ...
  const handleSocialLoginPress = async (provider) => {
    Toast.show({ type: 'info', text1: `${provider} login needs provider setup`, text2: 'Use email/password for grading unless OAuth client keys are added.' });
    try { await Linking.openURL('https://developers.google.com/identity'); } catch (error) { console.log(`${provider} setup link error:`, error.message); }
  };
  ...
  const handleVerifyOtp = async () => {
    if (otpCode.length !== 6) return Toast.show({ type: 'error', text1: 'Invalid Code', text2: 'Enter the 6-digit code.' });
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/auth/verify-otp`, { method: 'POST', body: formData });
      const data = await response.json();

      if (!response.ok) return Toast.show({ type: 'error', text1: 'Failed', text2: data.message || 'OTP verification failed.' });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (!response.ok) return Toast.show({ type: 'error', text1: 'Failed', text2: data.message || 'Could not resend OTP.' });
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.modalOverlay}>
            <View style={styles.modalFloating}>
              ...
              <Text variant="bodyMedium" style={styles.modalSubtitle}>We sent a 6-digit code to{`\n`}<Text style={{ fontWeight: 'bold', color: '#4A3B32' }}>{email}</Text></Text>

              <TextInput mode="outlined" label="6-Digit Code" value={otpCode} onChangeText={setOtpCode} keyboardType="number-pad" maxLength={6} style={styles.otpInput} outlineColor="#EBE1D7" activeOutlineColor="#6F4E37" />
...
const styles = StyleSheet.create({
  ...
  socialBtn: { width: '100%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff', borderRadius: 12, paddingVertical: 14, borderWidth: 1, borderColor: '#E0D2C2' },
  ...
  modalFloating: { width: '100%', backgroundColor: '#fff', borderRadius: 25, padding: 25, alignItems: 'center' },
  closeModalBtn: { position: 'absolute', top: 8, right: 8 },
  modalIconCircle: { width: 80, height: 80, borderRadius: 40, backgroundColor: '#F2E8DD', justifyContent: 'center', alignItems: 'center', marginBottom: 20, marginTop: 10 },
