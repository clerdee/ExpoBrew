...
const PICKUP_BRANCHES = [
  "Main Branch - TUP Taguig, Western Bicutan, Taguig City",
  "2nd Branch - Malacanang Village, Paranaque City",
  "Outlet Branch - Near Silangan Elementary School, Upper Bicutan, Taguig City"
];
...
export default function PlaceOrderPage({ route, navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (uStr) {
        ...
        setUser({ ...u, addresses: u.addresses?.length ? u.addresses : [] });
      }
    })();
  }, []);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      await db.runAsync('UPDATE cart_table SET cart_data = ? WHERE id = 1;', JSON.stringify([]));
    ...
    <View style={styles.bg}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.rowOpt}>
          <TouchableOpacity style={[styles.optCard, method==='Pickup'&&styles.optOn]} onPress={()=>setMethod('Pickup')}>
            <MaterialCommunityIcons name="storefront-outline" size={28} color={method==='Pickup'?'#6F4E37':'#888'} />
            <Text style={[styles.optTxt, method==='Pickup'&&styles.optTxtOn]}>Pickup</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.optCard, method==='Delivery'&&styles.optOn]} onPress={()=>setMethod('Delivery')}>
            <MaterialCommunityIcons name="bike-fast" size={28} color={method==='Delivery'?'#6F4E37':'#888'} />
            <Text style={[styles.optTxt, method==='Delivery'&&styles.optTxtOn]}>Delivery</Text>
          </TouchableOpacity>
        </View>
        ...
        {method === 'Delivery' && (
          <Card style={styles.card}>
            ...
            {user?.addresses?.map((addr, i) => (
              <TouchableOpacity key={i} style={styles.radioRow} onPress={()=>setAddrType(i)}>
                <RadioButton value={i} status={addrType===i?'checked':'unchecked'} color="#6F4E37" onPress={()=>setAddrType(i)} />
                <View style={{flex:1}}><Text style={styles.addrTxt}>{addr}</Text></View>
              </TouchableOpacity>
            ))}
            <TouchableOpacity style={styles.radioRow} onPress={()=>setAddrType('custom')}>
              <RadioButton value="custom" status={addrType==='custom'?'checked':'unchecked'} color="#6F4E37" onPress={()=>setAddrType('custom')} />
              <Text style={styles.addrTxt}>Use a different address</Text>
            </TouchableOpacity>
        ...
        {method === 'Pickup' && (
          <Card style={styles.card}>
            ...
            {PICKUP_BRANCHES.map((b, i) => (
              <TouchableOpacity key={i} style={styles.radioRow} onPress={()=>setBranch(b)}>
                <RadioButton value={b} status={branch===b?'checked':'unchecked'} color="#6F4E37" onPress={()=>setBranch(b)} />
                <View style={{flex:1}}><Text style={styles.addrTxt}>{b}</Text></View>
              </TouchableOpacity>
            ))}
          </Card>
        )}
          ...
          {['GCash', 'Credit / Debit Card', 'Cash on Delivery'].map((m) => {
            ...
            return (
              <TouchableOpacity key={m} style={styles.radioRow} onPress={()=>setMop(m)}>
                <RadioButton value={m} status={mop===m?'checked':'unchecked'} color="#6F4E37" onPress={()=>setMop(m)} />
                <MaterialCommunityIcons name={m==='GCash'?'wallet':m==='Credit / Debit Card'?'credit-card-outline':'cash'} size={24} color="#555" style={{marginHorizontal:10}} />
                <Text style={styles.addrTxt}>{m === 'Cash on Delivery' && method === 'Pickup' ? 'Pay at Counter' : m}</Text>
              </TouchableOpacity>
            );
          })}
        </Card>
        ...
        <Card style={styles.card}>
          {orderItems.map((item, i) => (
            <View key={i} style={styles.itemRow}>
              <View style={styles.itemQty}><Text style={styles.qtyTxt}>{item.qty}x</Text></View>
              <View style={{flex:1}}><Text style={styles.itemTxt}>{item.name}</Text>{renderCust(item.customizations)}</View>
              <Text style={styles.itemTxt}>₱{(item.price*item.qty).toFixed(2)}</Text>
            </View>
          ))}
