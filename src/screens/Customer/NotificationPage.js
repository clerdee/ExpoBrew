...
export default function NotificationsPage({ navigation }) {
  ...
  const promos = notifs.filter(n => n.type === 'Promo'), updates = notifs.filter(n => n.type === 'Order');
  ...
  return (
    <View style={styles.bg}>
      <View style={styles.head}><IconButton icon="arrow-left" onPress={() => navigation.goBack()} /><Text style={styles.hTitle}>Notifications</Text><View style={{width:48}} /></View>
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true); fetchData();}} />}>
        ...
        <View style={styles.secHead}><Text style={styles.secTitle}>Order Updates</Text><TouchableOpacity onPress={markAllRead}><Text style={styles.readAll}>Mark all as read</Text></TouchableOpacity></View>
        {loading ? <ActivityIndicator color="#6F4E37" style={{marginTop:20}} /> : <FlatList data={updates} scrollEnabled={false} keyExtractor={i=>i._id} renderItem={renderUpdate} ListEmptyComponent={<Text style={styles.empty}>No updates yet</Text>} />}
      </ScrollView>
    </View>
  );
}
...
const styles = StyleSheet.create({
  bg: { flex: 1, backgroundColor: '#FFF' }, head: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 50, paddingBottom: 10, borderBottomWidth: 0.5, borderColor: '#EEE' },
  hTitle: { fontSize: 18, fontWeight: 'bold', color: '#4A3B32' }, promoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15 },
  row: { flexDirection: 'row', alignItems: 'center' }, pTitle: { fontSize: 16, fontWeight: '600', color: '#333' }, pSub: { fontSize: 13, color: '#888', marginTop: 2 },
  ...
  divider: { height: 10, backgroundColor: '#F5F5F5' }, secHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomWidth: 0.5, borderColor: '#F0F0F0' },
  secTitle: { fontSize: 14, color: '#666' }, readAll: { fontSize: 14, color: '#6F4E37', fontWeight: '600' }, item: { flexDirection: 'row', padding: 15, borderBottomWidth: 0.5, borderColor: '#F0F0F0' },
  unread: { backgroundColor: '#FDF7F2' }, iconCircle: { width: 45, height: 45, borderRadius: 22.5, backgroundColor: '#F5F5F5', justifyContent: 'center', alignItems: 'center' },
  iTitle: { fontWeight: 'bold', color: '#333', fontSize: 15 }, iMsg: { color: '#666', fontSize: 13, marginTop: 4 }, iTime: { color: '#AAA', fontSize: 11, marginTop: 6 }, empty: { textAlign: 'center', marginTop: 40, color: '#CCC' }
});