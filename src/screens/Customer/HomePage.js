...
import { View, StyleSheet, FlatList, ScrollView, TouchableOpacity, Dimensions, ActivityIndicator } from "react-native";
...
// Aligned with your backend Product model schema
...
export default function HomePage() {
  ...
  const renderHeader = () => (
    <View>
      <Header user={user} cartItemCount={2} onAvatarPress={handleAvatar} onCartPress={() => setCartVis(true)} onNotificationPress={() => {}} />
      ...
      <Card style={styles.banner}>
        ...
        <View style={styles.bannerOver}>
          <Text style={styles.bTitle}>SALE</Text><Text style={styles.bSub}>UP TO 50% OFF</Text>
  ...
  return (
    <View style={styles.container}>
      {loading ? <ActivityIndicator size="large" color="#6F4E37" style={{ flex: 1, justifyContent: 'center' }} /> : (
        <FlatList data={filtered} renderItem={({ item }) => <CardComponent item={item} onAddToCart={() => setCartVis(true)} />} keyExtractor={i => i._id.toString()} numColumns={2} columnWrapperStyle={styles.colWrap} ListHeaderComponent={renderHeader} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false} />
      )}
      <CartModal visible={cartVis} onClose={() => setCartVis(false)} />
...
const styles = StyleSheet.create({
  ...
  bannerImg: { height: 140 }, bannerOver: { position: "absolute", left: 20, justifyContent: "center" }, bTitle: { color: "#fff", fontWeight: "900", fontSize: 24, textShadowColor: "rgba(0,0,0,0.5)", textShadowRadius: 5 },
  bSub: { color: "#fff", fontSize: 14, marginBottom: 8, textShadowColor: "rgba(0,0,0,0.5)", textShadowRadius: 5 }, bBtn: { backgroundColor: "#fff", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, alignSelf: "flex-start" },
