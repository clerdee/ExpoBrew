            ...
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ width: '100%' }}>
              <View style={styles.sheet}>
                ...
                <View style={styles.chipWrap}>
                  {CATEGORIES.map(cat => (
                    <Chip key={cat} selected={tempCategory === cat} onPress={() => setTempCategory(cat)} style={[styles.filterChip, tempCategory === cat && styles.filterChipActive]} textStyle={tempCategory === cat ? styles.filterChipTextActive : styles.filterChipText}>{cat}</Chip>
                  ))}
                </View>
