...
export default function IndividualProductModal({ visible, onClose, product }) {
    ...
    const formatCust = (cust) => {
        if (!cust) return null;
        const vals = Object.values(cust).filter(Boolean);
        if (!vals.length) return null;
        return `Variation: ${vals.join(', ')}`;
    };
                ...
                <View style={styles.sheet}>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.infoWrap}>
                            ...
                            {/* DESCRIPTION PULLED FROM MONGODB */}
                        ...
                        {loading ? <ActivityIndicator color="#6F4E37" style={{ marginVertical: 30 }} /> : !reviews.length ? (
                            <View style={styles.emptyWrap}><MaterialCommunityIcons name="comment-quote-outline" size={40} color="#CCC" /><Text style={styles.empty}>No reviews yet. Be the first to try it!</Text></View>
                        ) : reviews.map((r, i) => (
                            <Card key={i} style={styles.reviewCard}>
                                <Card.Content style={styles.rRow}>
                                    ...
                                    <View style={styles.rInfo}>
                                        ...
                                        {/* SHOPEE STYLE VARIATION DISPLAY */}
