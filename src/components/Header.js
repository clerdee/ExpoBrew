...
import { useNavigation } from "@react-navigation/native"; // Using hook for cleaner access
  ...
  if (!isLoggedIn) return (
    <View style={styles.gWrapper}>
      <View style={styles.gContainer}>
        ...
        <MaterialCommunityIcons name="coffee" size={150} color="rgba(111,78,55,0.05)" style={styles.gDeco} />
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.header}>
      <TouchableOpacity style={styles.hLeft} activeOpacity={0.7} onPress={onAvatarPress}>
        {user.profileImage ? (
          <Avatar.Image size={42} source={{ uri: user.profileImage }} />
        ) : (
          <Avatar.Icon size={42} icon="account" color="#fff" style={{ backgroundColor: "#A0A0A0" }} />
        )}
...
const styles = StyleSheet.create({
  ...
  nDot: { position: "absolute", top: 10, right: 12, width: 9, height: 9, borderRadius: 5, backgroundColor: "#E74C3C", borderWidth: 1.5, borderColor: '#FFF' },
