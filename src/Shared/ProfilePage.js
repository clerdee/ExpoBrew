...
const ProfilePage = ({ navigation }) => {
    ...
    const loadUserData = async () => {
      try {
        ...
        if (userInfoString) {
          ...
          // 🌟 Load new fields if they exist in the database
          setPhone(user.phone || "");
  ...
  const pickFromGallery = async () => {
    ...
    if (!permission.granted) return Toast.show({ type: "error", text1: "Permission Required" });
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      // 🌟 You will send the new fields to your backend here!
        ...
        <View style={styles.content}>
          ...
          <View style={styles.avatarContainer}>
            <TouchableOpacity onPress={showImageOptions} activeOpacity={0.8}>
              {profileImage ? (
                <Avatar.Image size={110} source={{ uri: profileImage }} style={styles.avatarImage} />
              ) : (
          ...
          <TextInput 
            ...
            left={<TextInput.Icon icon="phone-outline" color="#888" />} 
          />
          <TextInput 
            label="Birthday (MM/DD/YYYY)" value={birthday} onChangeText={setBirthday} 
            mode="outlined" style={styles.input} outlineColor="#EBE1D7" activeOutlineColor="#6F4E37"
            left={<TextInput.Icon icon="cake-variant-outline" color="#888" />} 
          />
          <TextInput 
            label="Primary Address" value={address} onChangeText={setAddress} 
            mode="outlined" style={styles.input} outlineColor="#EBE1D7" activeOutlineColor="#6F4E37"
            left={<TextInput.Icon icon="map-marker-outline" color="#888" />} 
          />

          {/* Security */}
          <TextInput 
            label="New Password (Optional)" value={password} onChangeText={setPassword} 
            mode="outlined" style={styles.input} outlineColor="#EBE1D7" activeOutlineColor="#6F4E37"
          ...
          <Button 
            mode="contained" buttonColor="#6F4E37" style={styles.saveBtn} 
            contentStyle={styles.saveBtnContent} labelStyle={styles.saveBtnLabel} 
            onPress={handleUpdateProfile} loading={isLoading} disabled={isLoading}
          >
...
const styles = StyleSheet.create({
  ...
  scrollContent: { flexGrow: 1, paddingTop: 40, paddingBottom: 40 },
  ...
  avatarPlaceholder: { 
    width: 110, height: 110, borderRadius: 55, backgroundColor: "#EBE1D7", 
    justifyContent: "center", alignItems: "center", borderWidth: 3, 
    borderColor: "#fff", elevation: 4, shadowOpacity: 0.15, shadowRadius: 5 
  },
  avatarImage: { backgroundColor: "#EBE1D7", borderWidth: 3, borderColor: "#fff" },
  ...
  editBadge: { 
    ...
    alignItems: "center", borderWidth: 3, borderColor: "#FAF5F0" 
  },

  input: { backgroundColor: "#fff", marginBottom: 15 },
  saveBtn: { borderRadius: 12, marginTop: 15 },
  saveBtnContent: { height: 55 }, 
