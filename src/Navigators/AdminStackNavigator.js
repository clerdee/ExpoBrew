  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.foot}>
        <DrawerItem label="Log Out" icon={() => <MaterialCommunityIcons name="logout" size={24} color="#D32F2F" />} labelStyle={styles.logout} onPress={handleLogout} />
      </View>
    </View>
  );
}
...
export default function AdminStackNavigator() {
  return (
    <Drawer.Navigator
      ...
      screenOptions={{
        headerShown: false,
        drawerStyle: { backgroundColor: '#FAFAFA', width: 280 },
        drawerActiveBackgroundColor: '#4A2E1B',
        drawerActiveTintColor: '#FFF',
        drawerInactiveTintColor: '#555',
        drawerLabelStyle: { fontSize: 15, fontWeight: '600', marginLeft: -10 },
        drawerItemStyle: { borderRadius: 12, paddingHorizontal: 5, marginVertical: 4 },
      }}
    >
      ...
      <Drawer.Screen name="Settings" component={AdminProfile} options={{ drawerIcon: ({ color }) => <MaterialCommunityIcons name="account-cog" size={22} color={color} /> }} />
...
const styles = StyleSheet.create({
  ...
  aName: { color: '#FFF', fontSize: 22, fontWeight: 'bold', marginBottom: 2 },
  aEmail: { color: '#D3C4B7', fontSize: 14, marginBottom: 12 },
  badge: { backgroundColor: '#D4AF37', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 15, alignSelf: 'flex-start' },
  bTxt: { color: '#FFF', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 0.5 },
